...
app.get("/_core/run.json",function(req,res){
    // SECURITY!!!
    //console.log(nvent.baseEditPath);
    if (nvent.baseEditPath === null){
        throw error.create("AccessDeniedError");
    ...
    path.exists(filePath,function(exists){
        if (exists){
            fileUtil.isFolder(filePath,function(isFolder){
                if (isFolder){
                    throw error.create("NotImplementedError");
                } else {
                    var child = child_process.spawn("node",[filePath]);
                    var buffer = [];
                    child.stdout.on("data",function(data){
                        buffer.push(data);
                    });
                    child.on("exit",function(code){
                        res.send(core.success(buffer.join("")));
                    });
                    //res.send(core.success());
                    /*
                    fs.readFile(filePath,"binary",function(error,file){
